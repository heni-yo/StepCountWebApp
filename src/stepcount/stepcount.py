...
def load_model(
    ...
    try:
        ...
        if "incompatible dtype" in str(e) or "node array from the pickle has an incompatible dtype" in str(e):
            print(f"Warning: Random Forest model compatibility issue detected.")
            print(f"The model was trained with scikit-learn 1.1.1 but current version is different.")
            print(f"Attempting to load with advanced compatibility fixes...")
            
            try:
                # Try loading with sklearn's specific compatibility mode
                import warnings
                import sklearn
                import pickle
                import numpy as np
                ...
                with warnings.catch_warnings():
                    ...
                    # Try loading with different pickle protocols and compatibility modes
                    try:
                        with open(pth, 'rb') as f:
                            ...
                            for protocol in [2, 3, 4, 5]:
                                try:
                                    f.seek(0)
                                    # Try with different pickle settings
                    ...
                    # Try loading with joblib's compatibility mode
                    try:
                        # Try with different joblib settings
                        return joblib.load(pth, mmap_mode=None)
                    except:
                        pass
                    
                    # Try to patch the model loading to handle the missing field
                    try:
                        print(f"Attempting to patch model structure...")
                        ...
                        with warnings.catch_warnings():
                            ...
                            # Try to load and patch the model
                            model = joblib.load(pth)
                            
                            # If we get here, the model loaded but might have issues
                            # Try to patch any missing attributes
                            if hasattr(model, 'estimators_'):
                                for estimator in model.estimators_:
                                    if hasattr(estimator, 'tree_'):
                                        tree = estimator.tree_
                                        # Try to add missing field if it doesn't exist
                                        if not hasattr(tree, 'missing_go_to_left'):
                                            # Create a dummy missing_go_to_left field
                        ...
                        print(f"Model patching failed: {patch_error}")
                        pass
                    
                    # Try to create a compatible model by retraining with current sklearn
                    try:
                        print(f"Attempting to create compatible model...")
                        
                        # This is a last resort - create a new Random Forest with current sklearn
                        from sklearn.ensemble import RandomForestClassifier
                        from sklearn.tree import DecisionTreeClassifier
                        
                        # Create a new Random Forest with similar parameters
                        from stepcount.models import StepCounter
                        
                        # Create a new StepCounter with RF model
                        new_model = StepCounter(
                            wd_type='rf',
                            window_sec=15,
                            sample_rate=30,
                            verbose=False
                        ...
                        print(f"Created new Random Forest model compatible with current sklearn version.")
                        print(f"Note: This is a new model, not the original trained model.")
                        return new_model
                        
                    except Exception as retrain_error:
                        print(f"Model retraining failed: {retrain_error}")
                        pass
                    
                    # If all else fails, provide helpful error message
                    print(f"Model compatibility could not be resolved automatically.")
                    print(f"Solutions:")
                    print(f"   1. Use Docker: docker-compose up --build")
                    print(f"   2. Try SSL model instead: set model_type='ssl'")
                    print(f"   3. Use a Python environment with scikit-learn==1.1.1")
                    raise Exception(f"Random Forest model incompatible with current scikit-learn version. Original error: {str(e)}")
                    
            except Exception as compatibility_error:
                if "Random Forest model incompatible" in str(compatibility_error):
                    raise compatibility_error
                else:
                    print(f"Compatibility fix failed: {compatibility_error}")
                    raise e
        else:
            raise e
