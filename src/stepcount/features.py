...
def extract_features(xyz, sample_rate=100):
    ...
    if len(xyz) <= MIN_WINDOW_SEC * int(sample_rate):
        return {}
...
def autocorr_features(v, sample_rate):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):  # ignore invalid div warnings
        # Ensure nlags is an integer (required by stattools.acf)
        nlags = int(2 * sample_rate)
        u = np.nan_to_num(stattools.acf(v, nlags=nlags))
...
def fft_features(v, sample_rate, nfreqs=5):
    ...
    # Ensure nperseg and noverlap are positive integers (required by signal.welch)
    nperseg = max(1, int(sample_rate))
    noverlap = max(0, int(sample_rate // 2))
    ...
    # Handle cases where there aren't enough frequency bins
    available_freqs = min(len(powers) - 1, nfreqs)
    feats = {f"fft{i}": powers[i] if i < len(powers) else 0.0 for i in range(nfreqs + 1)}
...
def peaks_features(v, sample_rate):
    ...
    # Ensure distance is an integer (required by signal.find_peaks)
    distance = int(0.2 * sample_rate)
    peaks, peak_props = signal.find_peaks(u, distance=distance, prominence=0.25)
